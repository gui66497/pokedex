...
const analyzeInterval = 3000; // 设置调用 analyze 方法的间隔，单位为毫秒
export default {
  name: 'PokeScaner',
  props: {
    // 使用后置相机
  ...
  methods: {
    ...
    async tick() {
      if (this.$refs.video && this.$refs.video.readyState === this.$refs.video.HAVE_ENOUGH_DATA) {
        ...
        try {
          ...
          if (currentTime - lastAnalyzeTime > analyzeInterval) {
            await this.analyze(imageData);
    ...
    async analyze (imageData) {
      ...
      predictions.sort(this.sortByProperty("prob"));
      ...
      let who = predictions[0].label.replace(/\r|\n/ig,"");
      if (predictions[0].prob > 0.7) {
        this.toDetail(who);
      }
    },
    ...
    toDetail (who) {
      console.log("who", who);
      this.fullStop();
      this.$router.push({path: `/detail`, query: {name: who.toLowerCase()}});
    }
  },
  mounted () {
    this.setup();
